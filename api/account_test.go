	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResonse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				requireBody(t, recorder.Body, account)
			},
		},
		{
			name:      "Not Found",
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, "unauthorized userd", time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
...
func requireBody(t *testing.T, body *bytes.Buffer, account db.Account) {
